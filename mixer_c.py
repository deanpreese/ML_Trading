...
from common.common_func import calc_MSE, calc_reg_results, calc_reg_streaks, show_stats, simple_split_and_scale, calc_class_results
from models.wrapped_models import TunableCatBoostClassifier, TunableCatBoostRegressor, TunableXGBClassifier, TunableXGBRegressor, TunableLGBMRegressor, TunableLGBMClassifier
import logging
...
def process_model(exp_name, data, models, run_test_size, feature_list_size):
        ...
        for f, e in enumerate(models):
                ...
                X_data = X_data.drop(columns=['outputC'])
                #feature_columns = list(X_data.columns[:-1])
                ...
                accuracy, precision, recall, TN, FP, FN, TP, tot, predictions, pred_proba  = e.track_model(exp_name, True, e, X_train, 
                                                                                  y_train, X_test, y_test)  
                
                all_predict_data[model_run_uuid] = predictions
                k = f"{model_run_uuid}-proba"
                all_predict_data[k] = pred_proba.tolist()

                t = [model_run_uuid, accuracy, precision, recall, TN, FP, FN, TP, tot]
                end = time.time()
                print(f" {model_run_uuid} execution - {end-start}")
                ...
                outputs = [ modelname, score, tot, mse, rmse, score, fl_out, model_run_uuid, e.run_id ]        
        ...
        correctX, correctY, correctP, totalX, cxp, cyp, cpp, r_predictions, r_y_target = calc_class_results(all_predict_data, estimator_run_ids)
...
est_list = [  TunableXGBClassifier(),  TunableXGBClassifier(),  TunableCatBoostClassifier() ,
              TunableXGBClassifier(),  TunableXGBClassifier(), TunableLGBMClassifier()  ]
