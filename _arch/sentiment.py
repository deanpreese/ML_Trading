...
def get_model_and_tokenizer(model_name):
    ...
    # If the model only provides PyTorch weights, we use from_pt=True to convert automatically.
...
def analyze_sentiment_transformers(text, model_name):
    ...
    try:
        ...
        # Assign positive or negative polarity based on predicted label
...
def process_urls_for_texts(urls, texts, model_names, tag='p', class_name=None, id_name=None):
    ...
    for text in texts:
        ...
        # Compute an overall (composite) sentiment across all models for this text
        model_averages = [
            vals['average_sentiment_score'] for vals in text_scores.values()
            if vals['average_sentiment_score'] is not None
        ]
        composite_score = sum(model_averages)/len(model_averages) if model_averages else None

        results.append({
            'text': text,
            'model_scores': text_scores,
...
if __name__ == "__main__":
    ...
    texts = [
        "What is the current sentiment on where the stock market is going in the next week",
        #"How is gold market doing today",
        #"What is the sentiment on the CA Wildfires",
    ]
    ...
    model_names = [
        ...
        #'ahmedrachid/FinancialBERT-Sentiment-Analysis',  # <-- Only PyTorch weights
    ...
    for result in results:
        ...
        print("Model Name - Average Sentiment Score:")
        for model_name, scores in result['model_scores'].items():
            print(f"  {model_name}: {scores['average_sentiment_score']}")
        print(f"Composite Sentiment Score: {result['composite_sentiment_score']}\n")
