...
def objective_xgbrf_c(trial, X_train, y_train, X_val, y_val):
...
def objective_xgb_r(trial, X_train, y_train, X_val, y_val):
    params = {
        ...
        'n_estimators': trial.suggest_int('n_estimators', 100, 150, 500),
...
def objective_xgbrf_r(trial, X_train, y_train, X_val, y_val):
    params = {
        "objective": "reg:squarederror",
        "verbosity": 0,
        'lambda': trial.suggest_loguniform('lambda', 7.0, 17.0),
...
def study_xgbrf_r(X_train, y_train, X_val, y_val):
    print(" ")
    print("XGBoostRF_R Tuning")
    study_xgbrf_r = optuna.create_study(direction='minimize')
    f_xgb_r = lambda trial: objective_xgbrf_r(trial, X_train, y_train, X_val, y_val)
    study_xgbrf_r.optimize(f_xgb_r, n_trials=20)
    return study_xgbrf_r.best_trial.params
...
def study_xgbrf_c(X_train, y_train, X_val, y_val):
    print(" ")
    print("XGBoostRF_C Tuning")
    study_xgbrf_c = optuna.create_study(direction='minimize')
    f_xgbrf_c = lambda trial: objective_xgb_c(trial, X_train, y_train, X_val, y_val)
    study_xgbrf_c.optimize(f_xgbrf_c, n_trials=20)
    return study_xgbrf_c.best_trial.params
...
def main():
    ...
    for i in range(len(datafile)):
        ...
        study_xgbrf_r_best_trial = study_xgbrf_r(X_train, y_train_r, X_val, y_val_r)
        t = f"xgbrf_r_{post_pend}={study_xgbrf_r_best_trial}"
        param_list.append(t)
        
        study_xgbrf_c_best_trial = study_xgbrf_c(X_train, y_train_c, X_val, y_val_c)
        t = f"xgbrf_c_{post_pend}={study_xgbrf_c_best_trial}"
        param_list.append(t)
        
        study_xgb_r_best_trial = study_xgb_r(X_train, y_train_r, X_val, y_val_r)
        t = f"xgb_r_{post_pend}={study_xgb_r_best_trial}"
        param_list.append(t)
        
        study_cat_r_best_trial = study_cat_r(X_train, y_train_r, X_val, y_val_r)               
        t = f"cat_r_{post_pend}={study_cat_r_best_trial}"
        param_list.append(t)
        
        study_lgb_r_best_trial = study_lgb_r(X_train, y_train_r, X_val, y_val_r) 
        t = f"lgb_r_{post_pend}={study_lgb_r_best_trial}"
        param_list.append(t)
        
        study_cat_c_best_trial = study_cat_c(X_train, y_train_c, X_val, y_val_c)               
        t = f"cat_c_{post_pend}={study_cat_c_best_trial}"
        param_list.append(t)
        
        study_xgb_c_best_trial = study_xgb_c(X_train, y_train_c, X_val, y_val_c)
        t = f"xgb_c_{post_pend}={study_xgb_c_best_trial}"
        param_list.append(t)
        
        study_lgb_c_best_trial = study_lgb_c(X_train, y_train_c, X_val, y_val_c) 
        t = f"lgb_c_{post_pend}={study_lgb_c_best_trial}"
        param_list.append(t)
        
        with open('gen_params.py', 'a') as f:
